<template>
  <div class="linklist">
    <ul>
      <li v-for="(linkItem, i) in blockData.links" :key="`link-block-${i}`">
        <a :href="linkItem.url" v-if="linkItem.type === 'ext'" target="_blank">
          {{linkItem.text}}
        </a>
        <router-link :to="linkItem.url" v-if="linkItem.type === 'int'">
          {{linkItem.text}}
        </router-link>
      </li>
    </ul>
